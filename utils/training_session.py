from utils.common import init_model, init_loss, init_optimiser, train_model

class TrainingSession:
    def __init__(self, model, optimiser, criterion, config, training_step, training_kwargs=None, history=None, epoch=0):
        self.model = model
    ...
    def train(self, epochs, train_loader, val_loader):
        ...
        # shift epoch numbers so they continue from previous training
        for m in history["epoch_metrics"]:
            m["epoch"] += self.epoch
        for b in history["batch_losses"]:
            b["epoch"] += self.epoch
...
def create_training_session(images, labels, method, dropout_prob,  config, training_step,**kwargs):
    ...
    non_training_keys = {"epochs", "batch_size", "seed", "optimiser", "validation_fraction",
                         "early_stopping_patience", "early_stopping_min_delta", "reg_dropout"}
    training_kwargs = {k: v for k, v in kwargs.items() if k not in optimiser_keys and k not in non_training_keys}
    optimiser = init_optimiser(model, method, **optimiser_kwargs)
    # allow training steps to access hyperparameters stored in config
    if config:
